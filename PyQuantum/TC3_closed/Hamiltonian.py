...
from PyQuantum.TC.Operators.sigma_ij import sigma_ij
# ---------------------------------------------------------------------------------------------------------------------
...
class Hamiltonian(Matrix):
    ...
    def cut_states(self, capacity):
        ...
        for i in range(len(self.states)):
            en_1 = self.states[i][0] + \
                np.count_nonzero(np.array(self.states[i][2]) >= 1)
            en_2 = self.states[i][1] + \
                np.count_nonzero(np.array(self.states[i][2]) == 2)
    ...
    def H1(self, capacity, at_count, n_levels, wc, wa, g):
        ...
        for i in range(1, at_count+1):
            ...
            H1 += wa[i-1] * kron(kron(I_ph['0_1'], I_ph['1_2']), elem)
            self.H_atoms_symb += kron(kron(I_ph['0_1'], I_ph['1_2']), elem)
